 ...
 */
public class GroupGridItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public GroupGridItemDecoration(Context context, List<TypeBean> data,
                                   int titleHeight, int titleFontSize, GridLayoutManager manager) {
        super();
        mDatas = data;
        mPaint = new Paint();
        mBounds = new Rect();
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position >= 0) {
            if (position<lastPosition+3){
                outRect.set(0, mTitleHeight, 0, mTitleHeight);
            }else {
                String type = mDatas.get(position).getType();
                String typeLast = mDatas.get(position - 1).getType();
